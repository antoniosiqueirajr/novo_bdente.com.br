...
class CFF_Events_Parser
{
    ...
    public function parse_events_ids()
    {
        ...
        $remote_ical_file = wp_remote_get($this->_ical_url);

        if (wp_remote_retrieve_response_code($remote_ical_file) !== 200 || !isset($remote_ical_file['body']) || strpos($remote_ical_file['body'], 'BEGIN:VCALENDAR') === false) {
            $this->update_need_ical_url(true);
            return;
        }

        $file_content =  str_replace(['\r', '\n', '\r\n'], '', $remote_ical_file['body']);
        $events_ids = [];

        preg_match_all("'BEGIN:VEVENT(.*?)END:VEVENT'si", $file_content, $events_content_array);
        $this->_event_source_name = $this->extract_page_name_name($file_content);
        foreach ($events_content_array[1] as $key => $single_event_content) {
            $pastdue_events = $this->filter_event_date($single_event_content);
            ...
            if ($pastdue_events && $organizer_events) {
                ...
                array_push($events_ids,
                    $this->extract_event_id($file_content_array[1])
                );
            }
     ...
     * @since 4.3.7
     */
    public function extract_event_id( $event_url )
    {
     ...
     * @since 4.3.7
     */
    public function extract_organizer_name( $string )
    {
        $string =  trim(str_replace(['\r', '\n', '\r\n'], '', $string));
        $string = explode(':', $string);
        return stripslashes($string[0]) ?? false;
    }
     ...
     * @since 4.3.7
     */
    public function extract_page_name_name( $file_content )
    {
     ...
     * @since 4.3.7
     */
    public function filter_event_date( $file_content )
    {
    ...
    public function filter_event_organizer( $file_content )
    {
        ...
        $organizer =  $this->extract_organizer_name($file_content_array[1]);
