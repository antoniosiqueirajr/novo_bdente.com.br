	...
	public function set_feed_type_and_terms( $next_urls_arr_safe = null ) {
		...
		foreach ( $page_ids as $page_id ) {
			...
			if ( ! $cff_events_only || ( $cff_events_only && $cff_events_source == 'timeline' ) ) {
				...
				//VIDEOS ONLY
				if ( $cff_videos_only ) {
					$cff_posts_json_url = 'https://graph.facebook.com/v3.2/' . $videos_id . '/videos/?access_token=' . $access_token . '&fields=source,created_time,title,description,embed_html,format{picture},status&locale=' . $cff_locale . $cff_date_range . '&limit=' . $cff_post_limit;
				}
