...
class Content_Management
{
    ...
    public function replace_media( $old_attachment_id )
    {
        ...
        if ( !empty($new_attachment_id) && is_numeric( $new_attachment_id ) && $old_post_mime == $new_post_mime ) {
            ...
            $recently_replaced_media = ( isset( $extra_options['recently_replaced_media'] ) ? $extra_options['recently_replaced_media'] : array() );
            ...
            $extra_options['recently_replaced_media'] = $recently_replaced_media;
    ...
    )
    {
        $extra_options = get_option( 'admin_site_enhancements_extra', array() );
        $recently_replaced_media = ( isset( $extra_options['recently_replaced_media'] ) ? $extra_options['recently_replaced_media'] : array() );
        if ( is_admin() && in_array( $attachment_id, $recently_replaced_media ) ) {
    ...
    public function append_cache_busting_param_to_attachment_image_src( $image, $attachment_id )
    {
        $extra_options = get_option( 'admin_site_enhancements_extra', array() );
        $recently_replaced_media = ( isset( $extra_options['recently_replaced_media'] ) ? $extra_options['recently_replaced_media'] : array() );
        if ( is_admin() && !empty($image[0]) && in_array( $attachment_id, $recently_replaced_media ) ) {
    ...
    public function append_cache_busting_param_to_attachment_for_js( $response, $attachment )
    {
        ...
        if ( is_admin() && in_array( $attachment->ID, $recently_replaced_media ) ) {
