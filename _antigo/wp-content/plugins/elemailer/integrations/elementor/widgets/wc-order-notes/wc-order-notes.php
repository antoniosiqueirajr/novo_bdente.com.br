...
/**
 * WooCommerce email order notes widget class
 *
 * @author elEmailer
 *
 * @since 1.0.8
 */
class Elemailer_Widget_WC_Order_Notes extends Widget_Base {
	...
	public function get_name() {
		return 'elemailer-wc-order-notes';
	}
	...
	public function get_title() {
		return esc_html__( 'WooCommerce Order Notes', 'elemailer' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'intro_message',
			[
				'label' 		=> __( 'Intro Message', 'elemailer' ),
				'type' 			=> 'elemailer_wc_email_content',
				'placeholder' 	=> __( 'Type additional info here', 'elemailer' ),
				'label_block' 	=> true,
	...
	protected function render() {
		...
		//$intro_message = Base::elemailer_internal_smart_tag_parser($order, $settings['intro_message'], false );
		$intro_message =$settings['intro_message'];

		echo "<div class='elemailer-email-customer-note'>".wpautop( $intro_message );
		if ( ! empty( $notes ) ) {
			$latest_note = $notes[ max( array_keys( $notes ) ) ];
			echo "<p class='note'>" . wpautop( wptexturize( make_clickable( $latest_note ) ) ) . "</p>";
		} elseif ( elemailer_is_edit_mode() || elemailer_is_preview_mode() ) {
			echo "<p class='note'>This is a sample order note from admin which is made using public option. It will be replaced by real one in the email itself</p>";
		}
		echo "</div>";
	}
}
