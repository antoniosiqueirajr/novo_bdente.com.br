	...
	static function cff_get_json_data( $feed_options, $next_urls_arr_safe, $data_att_html, $is_customizer = false ) {
	    ...
	    foreach ( $page_ids as $page_id ) {
	        ...
	        if ($cff_events_only && $cff_events_source == 'eventspage'){
	            ...
	            //Events URL
	            $events_post_limit = isset( $feed_options['eventspostlimit'] ) && intval($feed_options['eventspostlimit']) <= 50 ? $feed_options['eventspostlimit'] : 50;
	        ...
	        if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
	            ...
	            if($cff_videos_only){
	                ...
	                $cff_posts_json_url = 'https://graph.facebook.com/v3.2/'.$videos_id.'/videos/?access_token='.$access_token.'&fields=published,source,updated_time,created_time,title,description,embed_html,format{picture}'.$from_field.',status&locale='.$cff_locale . $cff_date_range . '&limit=' . $cff_post_limit;
	            }
	            ...
	            //ALBUMS ONLY
                //ALBUMS NEW SYSTEM
	            if($cff_albums_only && $cff_albums_source == 'photospage' && !$album_workaround){
	        ...
	        if( !is_null( $next_urls_arr_safe ) ) {
	        	...
	        	if($cff_is_group || ($cff_albums_only && $cff_albums_source == 'photospage')){
	            ...
	            if( $cff_more_posts ){
		            ...
		            if( $feed_options['type'] !== 'albums' || !$album_workaround ){
		           ...
		           if($album_workaround){
                        if( isset( $next_urls_arr_safe ) && isset( $next_urls_arr_safe['page'] ) ){
                            $albums_posts = new CFF_Album_Posts( $page_id, $access_token, $cff_date_range, $cff_is_group, '',  $feed_options );
                            $page_number = intval( $cff_post_limit ) * intval( $next_urls_arr_safe['page'] );
                            $posts_json = wp_json_encode($albums_posts->get_albums($cff_post_limit, $page_number ));
                        }
	        ...
	        //Get first set of posts
	            ...
	            if ($cff_events_only && $cff_events_source == 'eventspage'){
					...
					//Test Here Nabil
					$transient_event = $feed_id  !== false ?  $feed_id  : $transient_name;
					$sh_atts = empty($data_att_html) ? "{&quot;feed&quot; : &quot;".$transient_event   ."&quot;}" : $data_att_html;
		            $events_json = CFF_Utils::cff_get_set_cache(
						$cff_events_json_url,
						$transient_event,
						$cff_cache_time,
						$cache_seconds,
	            ...
	            if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
                    ...
                    if( $album_workaround ){
                        $albums_posts = new CFF_Album_Posts( $page_id, $access_token, $cff_date_range, $cff_is_group, '',  $feed_options );
                        $posts_json = wp_json_encode( $albums_posts->get_albums( $cff_post_limit , 0) );
                    }
		...
		if( $cff_videos_only ){
			$FBdata_arr = self::check_published_videos($FBdata_arr);
		}
	        ...
	        if ($cff_events_only && $cff_events_source == 'eventspage'){
	            ...
	            if( empty($json_data->data) ){
	                ...
	                foreach ($json_data->data as $event )
	                {
	                    ...
	                    $cff_event_date = '';
	                    if (!empty($event_time)){
	                        $cff_event_date = '<p class="cff-date"><span class="cff-start-date" '.$cff_event_date_styles.'>' . CFF_Utils::cff_eventdate(strtotime($event->start_time), $cff_event_date_formatting, $cff_event_date_custom, $cff_event_timezone_offset, $cff_timezone) . '</span>';
	                        if( isset($event->end_time) ) $cff_event_date .= '<span class="cff-end-date" '.$cff_event_date_styles.'> - ' . CFF_Utils::cff_eventdate(strtotime($event->end_time), $cff_event_date_formatting, $cff_event_date_custom, $cff_event_timezone_offset, $cff_timezone) . '</span>';
	                    ...
	                    if( $cff_show_link ){
		                    ...
		                    $cff_share_email = 'mailto:?subject=Facebook&amp;body=' . urlencode($event_link) . '%20-%20' . rawurlencode( strip_tags($cff_event_title) . ' - ' . strip_tags($cff_event_date) );
		                        ...
		                        //Buy tickets link
		                        if($ticket_uri !== '' && $cff_show_link) $cff_post_item .= '<a href="' . $ticket_uri . '" target="_blank" '.$cff_nofollow.' ' . $cff_link_styles . '>'.stripslashes(__( $cff_buy_tickets_text, 'custom-facebook-feed' ) ).'</a><span class="cff-dot" ' . $cff_link_styles . '>&middot;</span>';
	                               ...
	                               $cff_post_text = CFF_Utils::print_template_part( 'item/post-text', get_defined_vars());
	                                ...
	                                if(  ( $cff_show_meta || $cff_lightbox_comments ) ){
                                        ...
                                        if( in_array('comments_replies', self::$license_tier_features) ){
                                            ...
                                            $cff_comments .= '<p class="cff-comments cff-comment-on-facebook" ' . $cff_meta_styles . '>';
                                            ...
                                            $cff_comments .= '<a href="' . $link . '" ' . $target.$cff_nofollow . ' ' . $cff_meta_link_styles . '><span class="cff-icon">' . $cff_comment_svg . '</span>' . $cff_translate_comment_on_facebook_text . '</a></p>';
                                            ...
                                            if ( $comment_count > $cff_comments_num ) {
                                                $cff_comments .= '<p class="cff-comments cff-show-more-comments" ' . $cff_meta_styles . '><a class="cff-show-more-comments-a" href="javascript:void(0);" '.$cff_meta_link_styles.'>'.$cff_translate_view_previous_comments_text.'</a></p>';
                                            }
