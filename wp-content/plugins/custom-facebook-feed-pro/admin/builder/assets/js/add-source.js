    ...
    methods : {
         ...
         */
         processFBConnect : function(){
            ...
            const urlString = new URL(finalUrl);
            const urlParams = new URLSearchParams(urlString.search);

            const postData = {
                wordpress_user : urlParams.get('wordpress_user'),
                vn : urlParams.get('vn'),
                v : urlParams.get('v'),
                state : "{'{url=" + appendURL + "}'}"
            }
            const redirectURL = urlString.origin + urlString.pathname
            self.submitConnectPostForm(postData, redirectURL);
        },
